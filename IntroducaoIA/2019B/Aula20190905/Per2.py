...
flag_fim = 0 # flag que indica fim da execucao do laco
cont     = 0 # contatdor
...
while flag_fim == 0:
    ...
    for a in dataset:
        tmp_p = len(a) - 1
        if (calcula_erro(funcao_ativacao(soma(a, vt_w)), a[tmp_p]) != 0):
            flag_fim = 0
            corrige_pesos(a, vt_w, calcula_erro(funcao_ativacao(soma(a, vt_w)), a[tmp_p]), vl_ta)
